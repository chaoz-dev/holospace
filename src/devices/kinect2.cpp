...
std::vector<std::shared_ptr<Kinect2Device>> Kinect2DeviceFactory::create_devices() const {
	...
	size_t num_devices { static_cast<size_t>(freenect2_ptr->enumerateDevices()) };
	if (num_devices == 0)
...
Kinect2Device::Kinect2Device(
		std::shared_ptr<libfreenect2::Freenect2> freenect2_ptr, size_t cam_num,
		int cam_type, size_t timeout_sec) :
	...
	while (!device_ptr) {
		...
		std::chrono::high_resolution_clock::time_point end {
				std::chrono::high_resolution_clock::now() };
		long int duration { std::chrono::duration_cast<std::chrono::seconds>(
				(end - start)).count() };
...
bool Kinect2Device::start(size_t timeout_sec) {
	...
	if (!registration_ptr)
		registration_ptr = std::unique_ptr<libfreenect2::Registration> {
				new libfreenect2::Registration(device_ptr->getIrCameraParams(),
...
bool Kinect2Device::read_frame(Kinect2Frame & frame, size_t timeout_sec) const {
	assert(frame_listener_ptr);
	assert(registration_ptr);
...
void Kinect2Device::set_frame_listener(int cam_type) {
	...
	frame_listener_ptr = std::unique_ptr<libfreenect2::SyncMultiFrameListener>(
